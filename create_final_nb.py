...
# IFT3395 Competition 2 - Final Optimized Ensemble
    ...
    def fit(self, x: np.ndarray, y: np.ndarray, *, class_weights: np.ndarray, epochs: int, batch_size: int, val_data: Optional[Tuple[np.ndarray, np.ndarray]] = None, raw_data: Optional[np.ndarray] = None, augment: bool = False, patience: int = 40, warmup_epochs: int = 10) -> Dict[str, List[float]]:
        ...
        for epoch in range(epochs):
            if epoch < warmup_epochs:
                ...
                lr_scale = 0.5 * (1 + math.cos(math.pi * (epoch - warmup_epochs) / max(1, epochs - warmup_epochs)))
...
for idx, cfg in enumerate(sorted_configs):
    ...
    train_idx_full, val_idx_full = train_val_split_indices(len(x_std), val_ratio=0.1, seed=SEED * 10 + idx)
    model.fit(
...
submission_path = Path('submission_final_ensemble.csv')
...
nbf.write(nb, 'ift3395_final_ensemble.ipynb')
print('Notebook created: ift3395_final_ensemble.ipynb')
